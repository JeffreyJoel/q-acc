...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      animate={{
        backdropFilter: visible ? "bg-opacity-1 backdrop-blur-lg backdrop-filter" : "none",
        boxShadow: visible
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-transparent p-4 lg:flex dark:bg-transparent",
        visible && "bg-white/80 dark:bg-neutral-900/90",
        className,
      )}
    >
      {children}
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-50 mx-auto flex w-full max-w-[calc(100vw-2rem)] rounded-full flex-col items-center justify-between bg-transparent px-0 py-3 lg:hidden",
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-4 py-2 rounded-md bg-white button bg-white text-black font-semibold relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block text-center";
