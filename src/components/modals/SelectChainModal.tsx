...
const SelectChainModal = ({
  ...
  return (
    ...
    >
      <div
        className={`bg-neutral-800 w-full max-h-[100vh] overflow-x-hidden overflow-y-auto relative p-4 md:max-h-[90vh] md:rounded-xl md:shadow-lg md:max-w-[650px] md:p-6 font-redHatText border`}
      ...
      >
        <div className='flex gap-4  items-center justify-between pb-4'>
          <div className=' flex gap-4 items-center'>
            ...
            <h2 className='text-base font-semibold text-foreground leading-6 '>
              {showAllNetworks
                ? 'Select a Chain'
        ...
        {!showAllNetworks ? (
          <div className=' flex flex-col gap-5 font-redHatText '>
            <div className='grid grid-flow-row-dense  grid-cols-6 sm:grid-cols-9  gap-3'>
              <div
                className={`col-span-1 sm:col-span-4 h-[50px] flex py-[3px] px-2 justify-start cursor-pointer items-center border rounded-lg gap-2 ${POLYGON_POS_CHAIN_ID === selectedChain.id ? ' border-2 border-peach-400 bg-peach-400/10' : 'border-border hover:border-peach-400/50'} `}
                onClick={() => {
              ...
              {displayedNetworks.map(chain => (
                <div
                  key={chain.chainId}
                  className={`flex p-2 gap-2 items-center border w-[50px] h-[50px] rounded-lg justify-center   cursor-pointer ${chain.chainId === selectedChain.id ? ' border-2 border-peach-400 bg-peach-400/10' : 'border-border hover:border-peach-400/50'}  `}
                  onClick={() => {
              ...
              {!showAllNetworks && (
                <div
                  className='col-span-3 border border-border flex px-3 py-3 items-center justify-center gap-2 rounded-lg cursor-pointer hover:bg-muted hover:border-peach-400/50'
                  onClick={() => setShowAllNetworks(true)}
                >
                  <span className='font-semibold text-muted-foreground text-sm leading-5'>
            ...
            <div className='relative'>
              <input
                className='w-full pl-4 pr-12 py-2 text-base bg-background border-2 border-border rounded-lg focus:outline-none focus:border-peach-400 focus:ring-2 focus:ring-peach-400/20'
                ...
                {selectedChain && tokenLoading ? (
                  ...
                  filteredTokens.map((token: SquidTokenType) => (
                    <div
                      ...
                      className='flex px-2 py-1  gap-4 items-center cursor-pointer hover:border-2 border-2 border-transparent  hover:border-peach-400 rounded-lg hover:bg-peach-400/10'
                    >
                      ...
                      <div className='flex justify-between flex-[1_0_0] items-center font-redHatText'>
                        <div className='flex gap-2 items-center'>
                          <div className='font-medium text-foreground leading-5 text-sm'>
                            {token.symbol}
                          </div>
                        ...
                        {/* <div className='flex px-2 py-[2px] gap-2 bg-muted rounded-lg'>
                          <span className='text-muted-foreground font-medium leading-5 text-sm'>
          ...
          <div>
            <div className='relative'>
              <div>
                <input
                  className='w-full pl-4 pr-12 py-2 text-base bg-background border-2 border-border rounded-lg focus:outline-none focus:border-peach-400 focus:ring-2 focus:ring-peach-400/20 my-3'
              ...
              <div
                className={`w-full flex p-2 cursor-pointer items-center border border-border rounded-lg gap-2 my-3 hover:border-peach-400/50 hover:bg-peach-400/10`}
                onClick={() => {
              ...
              >
                ...
                <div className='flex flex-col gap'>
                  ...
                  <span className='text-[14px] text-muted-foreground text-sm'>
                    Save more on gas fees!
                  </span>
                </div>
              </div>
              <div className='flex flex-col gap-3  max-h-[500px] overflow-y-auto'>
                {filteredChains.map((chain: any) => (
                  ...
                  >
                    <div
                      className={`flex items-center gap-2 p-2 flex-[1_0_0] cursor-pointer border-2 border-transparent hover:border-peach-400 rounded-lg hover:bg-peach-400/10 `}
                    >
                      <img
                        className='rounded-full'
                        src={chain.chainIconURI}
