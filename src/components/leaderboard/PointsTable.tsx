  ...
  return (
    <div className="flex flex-col">
      ...
      {filteredData.find(item => item.isCurrentUser) && (
        <div className="my-6">
          <h2 className="text-base font-medium mb-3 font-tusker-8">Your Ranking</h2>
          {filteredData.filter(item => item.isCurrentUser).map((item) => (
            <div
              key={item.id}
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  ...
                  ${item.rank <= 3 ? "bg-peach-400 text-black" : "bg-[#3a3a3a] text-white"}
                  font-bold text-2xl
                `}
                >
                  {item.rank}
                ...
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <Avatar className="h-12 w-12 border-2 border-[#3a3a3a]">
                      <AvatarImage src={item.avatar} alt={item.name} className="object-cover h-full w-full rounded-full" />
              ...
              <div className="text-xl font-mono font-semibold">{item.points.toLocaleString()}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="space-y-3 my-6">
        <h2 className="text-base font-medium mb-3 font-tusker-8">Leaderboard</h2>
        {filteredData.filter(item => !item.isCurrentUser).map((item) => (
          <div
            key={item.id}
            className={`
              ...
              ${item.isCurrentUser ? "border border-peach-400" : ""}
            `}
          >
            <div className="flex items-center gap-4">
              <div
