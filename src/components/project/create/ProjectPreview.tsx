...
const ProjectPreview: FC<ProjectPreviewProps> = ({ formData, onClose, onEdit }) => {
  const socialMediaData = Object.entries(formData)
    .filter(([key, value]) => 
      value && 
      ['website', 'facebook', 'twitter', 'linkedin', 'discord', 'telegram', 'instagram', 'reddit', 'youtube', 'farcaster', 'lens', 'github'].includes(key)
    )
    .map(([key, value]) => {
      ...
      <div className="bg-neutral-900 rounded-lg max-w-6xl max-h-[90vh] w-full overflow-y-auto">
        ...
        <div className="sticky top-0 bg-neutral-900 border-b border-neutral-700 p-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-neutral-400">How your project will appear</span>
          </div>
            ...
            {formData.banner ? (
              ...
              <div className="w-full h-full flex items-center justify-center bg-gradient-to-r from-neutral-800 to-neutral-700">
                <div className="text-center">
                  <div className="text-neutral-500 mb-2">No banner uploaded</div>
                  <div className="text-sm text-neutral-600">1200x600px recommended</div>
                </div>
              </div>
            )}
              ...
              {formData.team && formData.team.length > 0 && (
                <TabsContent value="team" className="mt-0">
                  <div className="flex flex-wrap justify-center gap-4 py-4">
                    {formData.team.map((member: TeamMemberType, index: number) => (
                      <TeamMember
                        key={index}
                        member={{
                          name: member.name,
          ...
          <div className="mt-8 bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-amber-200 text-sm">
                This is a preview of how your project will appear once published. 
                Some features like donations and live data will only be available after creation.
              </p>
            </div>
          </div>
        </div>
